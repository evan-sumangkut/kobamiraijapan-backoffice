  ...
  <tbody>
    @foreach ($data as $d)
        <tr>
            ...
            <td>{{ $d->survey->name }}</td>
            <td>{{ $d->survey_total($d->responden_id, $d->survey_id) }}</td>
            <td>{{ $d->created_at }}</td>
            <td width="200">
                <a href="{{route('survey.recapitulation.responden',$d->id)}}" title="look data survey" type="button" class="btn btn-info btn-xs btn-modal-edit" stateId="{{$d->id}}" stateName="{{$d->responden->id}}">
